...
NEditor.init = function(element){
	NEditor.svg = element;
	NEditor.svg.classList.add("ConnectionSVG");
...
NEditor.getConnPos = function(elm){
	...
	const offset = NEditor.svg.getBoundingClientRect();
	pos.x += (elm.offsetWidth / 2) + 1.5 - offset.left; //Add some offset so its centers on the element
	pos.y += (elm.offsetHeight / 2) + 0.5 - offset.top;
	return pos;
};
...
NEditor.onConnDragMouseMove = function(e){
	e.stopPropagation(); e.preventDefault();
	const offset = NEditor.svg.getBoundingClientRect();
	if(NEditor.dragItem) NEditor.setQCurveD(NEditor.dragItem.path,NEditor.startPos.x,NEditor.startPos.y,e.clientX-offset.x,e.clientY-offset.y);
};
